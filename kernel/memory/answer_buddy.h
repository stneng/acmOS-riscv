...
void _buddy_split_page(page_t *original_page, uint64 target_order) {
    while (original_page->orders > target_order) {
        ...
        _buddy_clear_flag(original_page);
...
page_t *_buddy_alloc_page(uint64 order) {
    uint64 h_order = order;
    while (h_order < bd_max_size && bd_lists[h_order].nr_free == 0) h_order++;
    if (h_order >= bd_max_size) return NULL;
    _buddy_split_page(container_of_safe(bd_lists[h_order].list_head.next, struct page, list_node), order);
    if (bd_lists[order].nr_free == 0) return NULL;
    page_t *ret_page = container_of_safe(bd_lists[order].list_head.next, struct page, list_node);
    _buddy_get_specific_page(ret_page);
    return ret_page;
}
